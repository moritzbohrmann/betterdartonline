      ...
      <div
         ...
         <Alert.Root>
            <Alert.Trigger asChild>
               <Button className="bg-orange-500" disabled={currentLeg.scores.length > 1 ? isPlayer(currentLeg.scores.at(-1)?.player, profile) : true}>
                  Correction
               </Button>
            </Alert.Trigger>
            <Alert.Portal>
               ...
               <Alert.Content className="fixed left-[50%] top-[50%] z-20 translate-x-[-50%] translate-y-[-50%] data-[state=open]:animate-contentShow">
                  <Card className="flex h-72 w-96 flex-col items-center bg-dark-background">
                     ...
                     <input
                        className="h-8 w-20 rounded-md border-[1px] border-zinc-900 bg-dark-background px-4 text-center text-white-default outline-none"
                        type="number"
                        defaultValue={lastScore?.value}
                        min={0}
                        max={180}
         ...
         <Alert.Root>
            <Alert.Trigger asChild>
               <Button className="bg-red-500">Resign</Button>
            </Alert.Trigger>
            <Alert.Portal>
               <Alert.Overlay className="fixed inset-0 z-10 backdrop-blur-xl data-[state=open]:animate-overlayShow" />
               <Alert.Content className="fixed left-[50%] top-[50%] z-20 translate-x-[-50%] translate-y-[-50%] data-[state=open]:animate-contentShow">
